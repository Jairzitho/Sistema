...
function fok(e) {
	...
	if (fvalida(e.element())) {
		...
		if (0 < rows.length) {
			...
			/*Puntos*/
			if (!fis_int(document.frmdo.puntos.value) || 0 > parseInt(document.frmdo.puntos.value)) {
				Element.update(consola, 'Los puntos que se agregar&aacute;n al cliente deben ser enteros y mayor o igual a cero')
				Form.Element.activate(document.frmdo.puntos)
				return false
			}
